...
OrgSettingView=Ext.extend(Ext.Panel,{
	...
	initUI:function(){
		...
		this.demTabPanel=new Ext.TabPanel({
            ...
            listeners:{
            	tabchange: function(mainTab, tab){
    			}
            }
		});
		...
		//var orgGridPanel= this.getAllOrgGridPanel();
		var userGridPanel=this.getOrgUsersGrid();
		//右边Panel
	...
	removeRs : function(id) {
		$postDel({
			...
			ids:id,
			grid:this.userGridPanel
		});
	},
	//取得所有组织的GridPanel
	getAllOrgGridPanel:function(orgId){
		this.allOrgGridPanel = new HT.GridPanel({
					id:'orgPanel',
					tbar:[
						{
							text:'新建组织',
					...
					region : 'center',
					url:__ctxPath+'/system/depUsersAppUser.do?depId='+ (orgId ? orgId : ''),
					fields : [{name : 'userId',type : 'int'},
					          'username', 'fullname','email', 'depNames', 'posNames','roleNames',
					          {name : 'accessionTime'}],
					columns : [{
						header : 'userId',
						dataIndex : 'userId',
...
//		alert(posId)
//		if(posId==0) return;
		var panel=this.getPosDetailPanel.call(this,posId);
...
//		if(orgId==0) return;
		var panel=this.getOrgDetailPanel.call(this,orgId,orgName,orgType);
		this.centerPanel.removeAll();
		this.centerPanel.add(panel);
		this.centerPanel.doLayout();
	},
