		...
		.extend(
				Ext.FormPanel,
				{
					constructor : function(conf) {
						...
						this.initUI();
						UserFormPanel.superclass.constructor
								.call(
										this,
										{
											id : 'UserFormPanel_' + this.userId,
											title : (this.username ? this.username
													: '') + '员工信息编辑',
											autoScroll : true,
											width : 1000,
											tbar : [ {
						...
						var dialog = new Ext.ux.UploadDialog.Dialog(
								{
									...
									callback : function(obj) {
										...
										photo
												.setValue('/attachFiles/' + obj[0].filePath);
					...
					initUI : function() {
						...
						this.depGridPanel = new HT.EditorGridPanel( {
							...
							width : 400,
							height : 250,
							url : __ctxPath + '/system/findUserOrg.do?userId='
									+ this.userId,
							fields : [ 'userOrgId', 'orgId', 'orgName',
									'isPrimary' ],
							columns : [ {
								header : '单位名',
								dataIndex : 'orgName'
						...
						this.roleForSelPanel = new HT.GridPanel( {
							...
							title : '可选角色(双击选择)',
							url : __ctxPath
									+ '/system/getRolesAppUser.do?userId='
									+ this.userId,
						...
						this.roleSelectedPanel = new HT.GridPanel( {
							...
							title : '已选角色(双击删除)',
							url : __ctxPath
									+ '/system/getSelRolesAppUser.do?userId='
									+ this.userId,
							fields : [ 'roleId', 'roleName' ],
							columns : [ {
								header : '角色名',
						...
						this.posGridPanel = new HT.EditorGridPanel( {
							...
							showPaging : false,
							url : __ctxPath
									+ '/system/findUserPosition.do?userId='
									+ this.userId,
							fields : [ 'userPosId', 'posId', 'posName',
									'isPrimary' ],
							columns : [ {
								header : '岗位名称',
								dataIndex : 'posName'
						...
						this.rolePanel = new HT.HBoxPanel( {
							items : [ this.roleForSelPanel,
									this.roleSelectedPanel ]
						});

						//职位Panel
						this.posPanel = new HT.HBoxPanel( {
							items : [ this.posTreePanel, this.posGridPanel ]
						});
					...
					roleSelectedPanelDblClick : function(grid, rowIndex, e) {
						if (this.userId != '') {
							...
							Ext.Ajax.request( {
								url : __ctxPath
										+ '/system/delRoleAppUser.do?userId='
										+ this.userId,
								params : {
									roleId : roleId
								},
								...
								success : function(response, options) {
									...
									this.roleSelectedPanel.getStore().removeAt(
											rowIndex);
								}
							});
						} else {
					...
					depGridPanelDblClick : function(grid, rowIndex, e) {
						...
						if (userOrgId) {
							Ext.Ajax.request( {
								url : __ctxPath
										+ '/system/delUserOrg.do?userOrgId='
										+ userOrgId,
								method : 'Post',
								scope : this,
								success : function(response, options) {
									Ext.ux.Toast.msg('操作信息', '成功删除所属部门');
									this.depGridPanel.getStore().removeAt(
											rowIndex);
								}
							});
						} else {
					...
					save : function() {
						...
						for ( var i = 0; i < this.roleSelectedPanel.getStore()
								.getCount(); i++) {
							if (i > 0)
								roleIds += ',';
							roleIds += this.roleSelectedPanel.getStore().getAt(
									i).data.roleId;
						}

						for ( var i = 0; i < this.depGridPanel.getStore()
								.getCount(); i++) {
							orgIds
									.push(this.depGridPanel.getStore().getAt(i).data);
						}

						for ( var i = 0; i < this.posGridPanel.getStore()
								.getCount(); i++) {
							posIds
									.push(this.posGridPanel.getStore().getAt(i).data);
						}
