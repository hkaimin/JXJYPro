			...
			new Ext.grid.RowNumberer(), 
				{
						...
						renderer:function(value, metadata, record) {
							...
							if(isFolder==1){
							   ...
							   return '<img width="16" height="16" src="'+__ctxPath+'/images/flag/document/file.png"/>'+value+'</img>';
							}
						}
					},{
					    header:'类型',
						...
						renderer : function(value, metadata, record, rowIndex,
							...
							var str = '<a title="属性" value="" href="#" onclick="KnowledgeManageView.pro('
									+ editId +','+isFolder+ ')">属性</a>';
							if(rightMod==1&&isFolder==0){
								str += '<a title="编辑" value="" class="" href="#" onclick="KnowledgeManageView.edit('
										+ editId +','+isFolder+ ')">&nbsp;编辑</a>';
							}
							if(rightDel==1&&isFolder==0){
								str += '<a title="删除" value="" class="" href="#" onclick="KnowledgeManageView.deleteDocument('
										+ editId +')">&nbsp;删除</a>';
							}
							if (isFolder==1) {
								str += '<a title="编辑" value="" class="" href="#" onclick="KnowledgeManageView.edit('
										+ editId +','+isFolder+ ')">&nbsp;编辑</a>';
								str += '<a title="删除" value="" class="" href="#" onclick="KnowledgeManageView.deleteFolder('
										+ editId +')">&nbsp;删除</a>';
								str += '<a title="授权" value="" class="" href="#" onclick="KnowledgeManageView.privilege('
										+ editId +',\''+name+ '\')">&nbsp;授权</a>';
							}
							if(isFolder==0){
								str += '<a title="查看" value="" class="" href="#" onclick="KnowledgeManageView.detail('
										+ editId + ')">&nbsp;查看</a>';
							}
							return str;
		...
		this.gridPanel.addListener('rowdblclick', function(grid, rowindex, e) {
					grid.getSelectionModel().each(function(rec) {
								if(rec.data.isFolder==1){
								    ...
								    new DocumentDetailWin({docId:rec.data.fileId}).show();
								
								}
							},this);
				},this);
				
//		this.gridPanel.addListener('rowclick', function(grid, rowindex, e) {
//					grid.getSelectionModel().each(function(rec) {
//								if(rec.data.isFolder==1){
//									this.fileId=rec.data.fileId;
//									this.fileName=rec.data.fileName;
...
//		
		...
		if(!this.isSearching){
			...
			if (record != null) {
				...
				if(isFolder==1){
					...
					if(parentId==0){
						...
						this.getTitle().setTitle('['+parentName+']知识列表');
					}
			...
			if(node1){
				...
				if(node1.id==0){
					...
					this.getTitle().setTitle('['+node1.text+']在线文档列表');
				}
			}
		}else{
	...
	clickNode:function(node){
		if (node != null) {
			...
			if(node.id==0){
				...
				this.getTitle().setTitle('['+node.text+']知识列表');
			}
	    	this.fileId=node.id;
	...
	changPath:function(){
		 ...
		 Ext.getCmp('KnowledgeFilePathDisplayField').setValue(path);
	},
	upLevel:function(){
...
KnowledgeManageView.edit=function(fileId,isFolder){
	if(isFolder==1){
   		 new DocFolderForm({folderId:fileId}).show();
	}else{
	     new KnowledgeForm({docId:fileId}).show();
	}
};

KnowledgeManageView.detail=function(fileId){
   new DocumentDetailWin({docId:fileId}).show();
