...
MyTaskView.prototype.setup=function(){
		 ...
		 columns:[new Ext.grid.RowNumberer(),{
	      	...
	      	renderer:function(value,metadata,record,rowIndex,colIndex){
	      		...
	      		if(assignee==''){
	      			...
	      			str+='<button title="审核任务" class="btn-approvalTask" onclick="MyTaskView.nextStep(\''+taskId+'\',\''+activityName+'\')"></button>';
	...
	var grid = new Ext.grid.GridPanel({
      ...
      tbar : new Ext.Toolbar({
      	...
      	bbar : new HT.PagingBar({store : store,printable: true})
  });
...
MyTaskView.nextStep=function(taskId,activityName){
	...
	if(formView==null){
		formView=new ProcessNextForm({taskId:taskId,activityName:activityName});
