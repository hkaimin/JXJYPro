		...
		columns : [sm, new Ext.grid.RowNumberer(), {
					...
					renderer : function(value, metadata, record, rowIndex,colIndex) {
						...
						if(piId!=null && piId!=undefined && piId!=''){		
							str += '&nbsp;<button type="button" title="审批明细" value=" " class="btn-flowView" onclick="MyProcessRunView.detail('+
							runId+','+defId+',\''+ piId + '\',\''+ subject + '\')"></button>';
						}
						//若流程尚未结束，可进行编辑,及删除
						if(runStatus==0){
							str+='&nbsp;<button title="编辑" class="btn-edit" onclick="MyProcessRunView.edit('+runId+',\''+subject+'\')"></button>';
							str+= '&nbsp;<button title="删除" value=" " class="btn-del" onclick="MyProcessRunView.remove('+ runId + ')"></button>';
						}
						return str;
					}
				}],
	...
	var grid = new Ext.grid.GridPanel({
			...
			grid.addListener('rowdblclick', function(gridPanel,
					...
					gridPanel.getSelectionModel().each(function(rec) {
							MyProcessRunView.detail(rec.data.runId,rec.data.defId,rec.data.piId,rec.data.subject);
					});
			});
 ...
 * 
 */
MyProcessRunView.edit = function(runId,name) {
 ...
 * @param {} name
 */
MyProcessRunView.detail=function(runId,defId,piId,name){
	var contentPanel=App.getContentPanel();
