...
//				    alert(name+'   error !!');
//				}
//				var cmp=new Ext.form.NumberField({
//				    name:name,
//					renderTo:span,
//					allowBlank:element.getAttribute('txtisnotnull')==1?true:false,
//					width:width,
//					decimalPrecision:element.getAttribute('txtispssize')?element.getAttribute('txtispssize'):2
//					listeners:{
//						scope:this,
//					    'focus':function(cmp){
//					        cmp
...
//					var subTableId=document.getElementById('subPkName_'+this.runId);
		...
		Ext.each(fElements, function(element,index) {
	        ...
	        if(element.id=='entity_'+this.runId){
	            return;
	        }
			xtype=element.getAttribute('xtype');
			if(jsonData && jsonData[name]){
...
//     	        if(right==1){
     	          cmp.setReadOnly();
//     	        }
			    cmp.panel.render(span);
				if(element.value){
					this.hiddenF.setValue(element.value);
					 cmp.openDoc(element.value);
					 this.fileId=element.value;
				}
				this.officePanel=cmp;
				formPanel.add(cmp);
			}else if(xtype=='fileattach'){
				var span=document.createElement("span");
				var hiddenF=new Ext.form.Hidden({
...
//			    element.setAttribute('style','display:none;');
				...
				try{
					var p=document.createElement("span");
					p.setAttribute('style','width:'+width+'px; height:'+height+'px;');
					p.innerHTML=element.value?element.value:'';
					parent.appendChild(p);
				}catch(e){
				}
			}
