...
Ext.ux.form.FCKdesigner = function(config) { // add fckeditor config  fckconfig
	if(config.id==null){
		config.id='__fckdesigner_' + parseInt(1000*Math.random());
	}
	// object
	this.config = config;
	this.fckconfig = config;
	Ext.ux.form.FCKdesigner.superclass.constructor.call(this, config);
	this.instanceLoaded = false;
	this.instanceValue = '';
	this.editorInstance = undefined;
};

Ext.extend(Ext.ux.form.FCKdesigner, Ext.form.TextArea, {
			initEvents : function() {
			...
			onRender : function(ct, position) {
				...
				setTimeoutEx(loadFCKdesigner, 100, this.config.id, this.fckconfig); // setTimeoutEx
				//loadFCKdesigner(this.el.id,this.config);
				// support
				// arguments
			},
...
Ext.reg('fckdesigner', Ext.ux.form.FCKdesigner);

function loadFCKdesigner(element, config) {
	//加载fckEditor js
