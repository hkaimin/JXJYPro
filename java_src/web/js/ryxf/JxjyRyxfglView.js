/**
 * @author:
 * @class JxjyRyxfglView
 * @extends Ext.Panel
 * @description [JxjyRyxfgl]管理
 * @company 广州宏天软件有限公司
 * @createtime:
 */
JxjyRyxfglView = Ext.extend(Ext.Panel, {
	// 构造函数
...
//			},
	...
	rowClick : function(grid, rowindex, e) {
		grid.getSelectionModel().each(function(rec) {
			var xflx= rec.data.xflx
			if(xflx.indexOf('非')>=0){
			new JxjyRyxfglForm( {
				id : rec.data.id
			}).show();
			}else{
				Ext.ux.Toast.msg("信息", "不能编辑项目学分类型的记录！");
				return ;
			}
			new JxjyRyxfglForm( {
				id : rec.data.id
	...
	xfCommit : function() {
		...
		if(selectRecord[0].data.is_commit=='1'){
			Ext.ux.Toast.msg("信息", "已提交审核！");
			return;
		}
